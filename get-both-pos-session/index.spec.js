...
describe('get-both-pos-session', () => {
    ...
    it('should throw error on incorrect id field', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/both-pos-session/123`, {
                json: true,
                headers: {
    ...
    it('should return error when pos session does not exist.', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/both-pos-session/${uuid.v4()}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY
                }
    ...
    it('should return the document when all validation passes', async () => {
        const result = await request.get(`${helpers.API_URL}/api/v1/both-pos-session/${samplePOSSession._id}`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY
            }
