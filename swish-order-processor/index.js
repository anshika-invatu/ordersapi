...
module.exports = async (context, mySbMsg) => {
    ...
    try {
        ...
        if (!checkoutSession) {
            ...
            const logObj = {};
            logObj.massage = `checkoutSession does not exist in db for ${mySbMsg.payeePaymentReference}`;
        ...
        const logObj = {};
        logObj.massage = `send req to device api to perform action for ${mySbMsg.payeePaymentReference}`;
        CustomLogs(logObj, context);
        ...
        if (!iot) {
            const logObj = {};
            logObj.massage = `action not performed succesfully by device api for ${mySbMsg.payeePaymentReference}`;
            CustomLogs(logObj, context);
            return Promise.resolve();
        }
        ...
        if (!checkoutSession.receiverMobilePhone) {
            const logObj = {};
            logObj.massage = `phone number does not exist in checkoutsession doc with swish payment type for ${mySbMsg.payeePaymentReference}`;
            CustomLogs(logObj, context);
        ...
        if (checkoutSession.receiverMobilePhone) {
            ...
            checkoutSession.functionName = 'swish-order-processor';
        ...
        let customerInfoMasked = checkoutSession.receiverMobilePhone.replace(/.(?=.{4})/g, '');
