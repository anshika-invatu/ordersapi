...
module.exports = async (context, req) => {
    try {
        ...
        const retailTransactions = await collection.aggregate([
            { '$match': query },
            { '$unwind': '$lineItems' },
            { '$match': { 'lineItems.lineItemTypeCode': 'sales' }},
            { '$group': {
                _id: '$lineItems.sales.productID',
                doc: { '$first': { productName: '$$ROOT.lineItems.sales.productName', currency: '$$ROOT.currency' }},
                salesAmount: { '$sum': '$$ROOT.lineItems.amount' },
                numerOfProduct: { '$sum': 1 }
            }},
            { '$project': { _id: 0, productID: '$_id', productName: '$doc.productName', salesAmount: 1, numerOfProduct: 1, currency: '$doc.currency' }},
