...
sampleSession.pointOfServiceID = uuid.v4();

describe('Get Session by pointOfServiceID', () => {
    before(async () => {
    ...
    it('should throw error on incorrect id field', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/pointOfService/123/session`, {
                json: true,
                headers: {
    ...
    it('should throw 404 error if the documentId is invalid', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/pointOfService/${uuid.v4()}/session`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY
                }
    ...
    it('should get the document when all validation passes', async () => {
        const session = await request
            .get(`${helpers.API_URL}/api/v1/pointOfService/${sampleSession.pointOfServiceID}/session`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY
                }
        ...
        expect(session._id).to.eql(sampleSession._id);
