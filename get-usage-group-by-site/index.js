...
//Please refer the story BASE-653 for more details
...
module.exports = async (context, req) => {
    ...
    try {
        ...
        const result =  await collection.aggregate([
            { '$match': query },
            { '$sort': { createdDate: -1 }},
            { '$group': { _id: { siteID: '$siteID', siteName: '$siteName' },
                usageTotalVolume: { '$sum': '$$ROOT.usageTotalVolume' },
                usageTotalTimeMinutes: { '$sum': '$$ROOT.usageTotalTimeMinutes' }}},
            { '$project': { _id: 0, siteID: '$_id.siteID', siteName: '$_id.siteName', usageTotalVolume: 1, usageTotalTimeMinutes: 1 }}
        ]).toArray();
