...
describe('Get Retail Transaction by customerID', () => {
    before(async () => {
        sampleRetailTransaction.customerID = uuid.v4();
    ...
    it('should throw error on incorrect customerID field', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/customers/123-abc/retail-transaction`, {
                json: true,
                headers: {
    ...
    it('should throw error on incorrect merchantID field', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/customers/${uuid.v4()}/retail-transaction`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY
                }
    ...
    it('should return empty array if data not exist in db', async () => {
        ...
        const result = await request.get(`${helpers.API_URL}/api/v1/customers/${uuid.v4()}/retail-transaction`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY
            }
    ...
    it('should return the document when all validation passes', async () => {
        const result = await request.get(`${helpers.API_URL}/api/v1/customers/${sampleRetailTransaction.customerID}/retail-transaction`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY
            }
