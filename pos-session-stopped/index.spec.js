...
describe('Pos Session Stopped', () => {
    ...
    it('should return status code 400 when request body is null', async () => {
        try {
            await request.post(`${helpers.API_URL}/api/v1/pos-session-stopped`, {
                json: true,
                headers: {
            ...
            const response = {
                code: 400,
                description: 'You\'ve requested to pos session stopped but the request body seems to be empty. Kindly specify the request body in application/json format',
    ...
    it('should return error message with ok res when wrong pointOfService ID.', async () => {
        const result = await request.post(`${helpers.API_URL}/api/v1/pos-session-stopped`, {
        ...
        expect(result).to.eql({ description: 'Theres is an error when pos session event stopped.' });
