...
describe('Count up Session Started', () => {
    before(async () => {
    ...
    it('should return status code 400 when request body is null', async () => {
        try {
            await request.post(`${helpers.API_URL}/api/v1/count-up-session-started`, {
                json: true,
                headers: {
            ...
            const response = {
                code: 400,
                description: 'You\'ve requested to count up session started but the request body seems to be empty. Kindly specify the request body in application/json format',
    ...
    it('should return error message with 200 code when wrong pointOfSerciceID.', async () => {
        const result = await request.post(`${helpers.API_URL}/api/v1/count-up-session-started`, {
        ...
        expect(result).to.eql({ description: 'Theres is an error when count up session event started.' });
